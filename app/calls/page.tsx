...
export default function CallsPage() {
  ...
  const handleJoined = useCallback((s: SessionData) => setState({ phase: "in-meeting", session: s }), []);
  const handleError = useCallback((_m: string) => {}, []);
  const handleLeave = useCallback(() => setState({ phase: "ended" }), []);
  ...
  return (
    <main className="flex-1 flex items-center justify-center p-6 min-h-screen">
      ...
      {state.phase === "in-meeting" && <MeetingRoom session={state.session} onLeave={handleLeave} />}
      {state.phase === "ended" && (
